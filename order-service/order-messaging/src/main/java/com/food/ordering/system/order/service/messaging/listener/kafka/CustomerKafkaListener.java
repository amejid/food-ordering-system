...
public class CustomerKafkaListener implements KafkaConsumer<CustomerAvroModel> {
	...
	public void receive(@Payload List<CustomerAvroModel> messages, @Header(KafkaHeaders.RECEIVED_KEY) List<String> keys,
			@Header(KafkaHeaders.RECEIVED_PARTITION) List<Integer> partitions,
			@Header(KafkaHeaders.OFFSET) List<Long> offsets) {
		log.info("{} number of customer create messages received with keys {}, partitions {} and offsets {}",
				messages.size(), keys.toString(), partitions.toString(), offsets.toString());

		messages.forEach(customerAvroModel -> this.customerMessageListener
			.customerCreated(this.orderMessagingDataMapper.customerAvroModeltoCustomerModel(customerAvroModel)));
	}
