...
public class RestaurantApprovalOutboxScheduler implements OutboxScheduler {
	...
	public void processOutboxMessage() {
		...
		if (outboxMessagesResponse.isPresent() && !outboxMessagesResponse.get().isEmpty()) {
			...
			outboxMessages.forEach(outboxMessage -> this.restaurantApprovalRequestMessagePublisher
				.publish(outboxMessage, this::updateOutboxStatus));
	...
	private void updateOutboxStatus(OrderApprovalOutboxMessage orderApprovalOutboxMessage, OutboxStatus outboxStatus) {
		...
		this.approvalOutboxHelper.save(orderApprovalOutboxMessage);
		log.info("OrderApprovalOutboxMessage with outbox id: {} updated to status: {}",
