...
public class PaymentOutboxHelper {
	...
	public Optional<List<OrderPaymentOutboxMessage>> getPaymentOutboxMessageByOutboxStatusAndSagaStatus(
		...
		return this.paymentOutboxRepository.findByTypeAndOutboxStatusAndSagaStatus(SagaConstants.ORDER_SAGA_NAME,
	...
	public Optional<OrderPaymentOutboxMessage> getPaymentOutboxMessageBySagaIdAndSagaStatus(UUID sagaId,
		...
		return this.paymentOutboxRepository.findByTypeAndSagaIdAndSagaStatus(SagaConstants.ORDER_SAGA_NAME, sagaId,
				sagaStatus);
	}

	@Transactional
	public void save(OrderPaymentOutboxMessage orderPaymentOutboxMessage) {
		OrderPaymentOutboxMessage response = this.paymentOutboxRepository.save(orderPaymentOutboxMessage);
	...
	public void deletePaymentOutboxMessageByOutboxStatusAndSagaStatus(OutboxStatus outboxStatus,
			SagaStatus... sagaStatus) {
		this.paymentOutboxRepository.deleteByTypeAndOutboxStatusAndSagaStatus(SagaConstants.ORDER_SAGA_NAME,
				outboxStatus, sagaStatus);
	}

	private String createPayload(OrderPaymentEventPayload paymentEventPayload) {
		try {
			return this.objectMapper.writeValueAsString(paymentEventPayload);
		}
		catch (JsonProcessingException ex) {
			log.error("Could not serialize OrderPaymentEventPayload object for order id: {}",
					paymentEventPayload.getOrderId(), ex);
			throw new OrderDomainException("Could not serialize OrderPaymentEventPayload object for order id: "
					+ paymentEventPayload.getOrderId(), ex);
		}
	}
