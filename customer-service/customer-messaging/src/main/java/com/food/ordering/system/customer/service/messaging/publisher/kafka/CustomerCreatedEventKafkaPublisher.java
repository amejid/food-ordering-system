...
public class CustomerCreatedEventKafkaPublisher implements CustomerMessagePublisher {
	...
	public void publish(CustomerCreatedEvent customerCreatedEvent) {
		...
		try {
			CustomerAvroModel customerAvroModel = this.customerMessagingDataMapper
				.paymentResponseAvroModelToPaymentResponse(customerCreatedEvent);

			this.kafkaProducer.send(this.customerServiceConfigData.getCustomerTopicName(), customerAvroModel.getId(),
					customerAvroModel,
					getCallback(this.customerServiceConfigData.getCustomerTopicName(), customerAvroModel));
			...
			log.error("Error while sending CustomerCreatedEvent to kafka for customer id: {}," + " error: {}",
					customerCreatedEvent.getCustomer().getId().getValue(), ex.getMessage());
		}
	}
